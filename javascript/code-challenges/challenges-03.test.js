...
const typeNum = (arr) => {
  let newArr = arr.filter(thing => typeof thing === 'number');
  return newArr;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let newArr = arr.filter(data => data.baseStat > minBaseStat);
  return newArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  let childrenChar = arr.filter(character => !Boolean(character.children));
  return childrenChar;
};
...
const evenOddNumericValues = (arr) => {
  let numArr = typeNum(arr);
  let stringArr = numArr.map(number => {
    if (number % 2 == 0) {
      return "even"
    } else if (number % 2 != 0) {
      return "odd"
