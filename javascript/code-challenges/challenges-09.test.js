...
const hasChildrenEntries = (arr, character) => {
  let hasChildren = false;
  let temp = arr.map(obj => Object.entries(obj).flat());
  for (let i = 0; i < temp.length; i++) {
    if (temp[i].includes(character) && temp[i].includes('children')) {
      hasChildren = true;
    }
  }
  return hasChildren;
...
const totalCharacters = (arr) => {
  let total = []
  let noHouse = arr.map(obj => delete obj.house);
  arr.forEach(obj => {
    total.push(Object.values(obj).filter(value => value))
  })
  return total.flat(2).length
};
