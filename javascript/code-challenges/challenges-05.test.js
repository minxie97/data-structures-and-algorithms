...
const countNumberOfChildren = (arr) => {
  arr = arr.filter(char => char.children);
  return arr.reduce((acc, curr) => {
    acc = acc + countNumberOfElements(curr.children);
    return acc;
  }, 0)
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((acc, curr) => {
    isPrime(curr) === true ? acc = acc + 1 : acc = acc + 0;
    return acc;
  }, 0)
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((acc, curr) => {
    statName === curr.stat.name ? acc = curr : acc = acc;
    return acc;
  }, null)
};
...
const extractChildren = (arr) => {
  arr = arr.filter(char => char.name.toLowerCase().includes('a') && char.children);
  return arr.reduce((acc, curr) => {
